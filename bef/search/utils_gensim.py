    ...
    def _synched(func):
        ...
        def _synchronizer(self, *args, **kwargs):
            ...
            with tlock:  # use lock as a context manager to perform safe acquire/release pairs
                logger.debug("acquired lock %r for %s",
                             tlockname, func.__name__)
                result = func(self, *args, **kwargs)
                logger.debug("releasing lock %r for %s",
                             tlockname, func.__name__)
                return result
...
class SaveLoad(object):
    ...
    def _load_specials(self, fname, mmap, compress, subname):
        ...
        def mmap_error(obj, filename):
            return IOError(
                'Cannot mmap compressed object %s in file %s. ' % (
                    obj, filename)
        ...
        for attrib in getattr(self, '__scipys', []):
            ...
            if compress:
                ...
                sparse.data = np.load(
                    subname(fname, attrib, 'data'), mmap_mode=mmap)
    ...
    def _adapt_by_suffix(fname):
        ...
        """
        compress, suffix = (True, 'npz') if fname.endswith(
            '.gz') or fname.endswith('.bz2') else (False, 'npy')
        ...
        for attrib, val in iteritems(self.__dict__):
            ...
            if hasattr(val, '_save_specials'):
                ...
                restores.extend(val._save_specials(
                    cfname, None, sep_limit, ignore, pickle_protocol, compress, subname))
    ...
    def save(self, fname_or_handle, separately=None, sep_limit=10 * 1024**2, ignore=frozenset(), pickle_protocol=2):
        ...
        try:
            ...
            self._smart_save(fname_or_handle, separately, sep_limit,
                             ignore, pickle_protocol=pickle_protocol)
...
def chunkize_serial(iterable, chunksize, as_numpy=False, dtype=np.float32):
    ...
    while True:
        if as_numpy:
            ...
            wrapped_chunk = [[np.array(doc, dtype=dtype)
    ...
    def run(self):
        ...
        while True:
            ...
            try:
                ...
                qsize = '?'
            logger.debug("prepared another chunk of %i documents (qsize=%s)", len(
                wrapped_chunk[0]), qsize)
...
if os.name == 'nt' or (sys.platform == "darwin" and sys.version_info >= (3, 8)):
    def chunkize(corpus, chunksize, maxsize=0, as_numpy=False):
        ...
        if maxsize > 0:
            ...
            warnings.warn(
                "detected %s; aliasing chunkize to chunkize_serial" % entity)
    ...
    def chunkize(corpus, chunksize, maxsize=0, as_numpy=False):
        ...
        if maxsize > 0:
            ...
            worker = InputQueue(q, corpus, chunksize,
                                maxsize=maxsize, as_numpy=as_numpy)
            worker.daemon = True
            worker.start()
            while True:
...
def sample_dict(d, n=10, use_random=True):
    ...
    """
    selected_keys = random.sample(list(d), min(
        len(d), n)) if use_random else itertools.islice(iterkeys(d), n)
