...
def parse_arguments(parser):
    parser.add_argument("--emb_src", type=str,
                        help="Path to source embeddings")
    parser.add_argument("--emb_tgt", type=str,
    ...
    parser.add_argument("--wp", action="store_true",
                        help="Use WP instead of P rotation estimation")
    parser.add_argument("--rotation", type=str,
                        help="Path to WP rotation to save")
    ...
    parser.add_argument("--bsz", default=40, type=int,
...
def frontend(args, Emb_U_, User_U, Emb_L_, User_L):
    ...
    if args.test:
        if args.pca:
            ...
            pca_reload_u = pk.load(
                open(args.pca_load_path + "/pca_emb_u.pkl", "rb"))
            Emb_U = pca_reload_u.transform(Emb_U_)

            pca_reload_l = pk.load(
                open(args.pca_load_path + "/pca_emb_l.pkl", "rb"))
...
def Wasserstein_Procrustes_Alignment(args, Emb_L, Emb_U, verbose=False, last_iter=False):
    ...
    R = align(x_src, x_tgt, R0.copy(), bsz=args.bsz, lr=args.lr, niter=args.niter, corres=corres,
              nepoch=args.nepoch, reg=args.reg, nmax=N_pts_used, verbose=verbose, last_iter=last_iter)
...
def top1(Xn, Yn, Ux, Uy):
    compute_unit = torch.device(
        'cuda:0' if torch.cuda.is_available() else 'cpu')
    n_emb = len(Xn)
    L = np.zeros(n_emb).astype(int)
    for i in range(n_emb):
        distances = torch.sum((torch.FloatTensor(Xn[i]).to(compute_unit).unsqueeze(0).repeat(
            n_emb, 1)-torch.FloatTensor(Yn).to(compute_unit))**2, dim=1).cpu().numpy()
        L[i] = np.argmin(distances)
...
def topn(Xn, Yn, Ux, Uy, n=1):
    ...
    for i in range(n_emb):
        distances = torch.sum((torch.FloatTensor(Xn[i]).to(compute_unit).unsqueeze(0).repeat(
            n_emb, 1)-torch.FloatTensor(Yn).to(compute_unit))**2, dim=1).cpu().numpy()
        L[i, :] = np.array([j[0] for j in sorted(
            [(k, y) for k, y in enumerate(distances)], key=lambda x:x[1])])[:n]
...
if __name__ == "__main__":
    ...
    print("Top {:3}:\t{:.2f} (speaker accuracy)\t {:.2f} (segment accuracy)".format(
        1, acc_U, acc_F))
    if args.top_k:
        for n in [3, 5, 10, len(User_L)]:
            acc_U, acc_F = topn(Emb_U, np.dot(
                Emb_L, WP_R), User_U, User_L, n=n)
            print("Top {:3}:\t{:.2f} (speaker accuracy)\t {:.2f} (segment accuracy)".format(
                n, acc_U, acc_F))
